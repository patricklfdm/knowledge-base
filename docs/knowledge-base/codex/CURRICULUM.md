...
| F07B | 表单如何把操作变成数据？ | F07A | label/name、提交、文字转换、错误反馈；网络加载状态留 F08/F11 |
| F08 | 一次 HTTP 请求发生了什么？ | F05、F07 | 方法、URL、headers、JSON、状态码 |
| F09 | 如何实现并校验创建接口？ | F06、F08 | 业务规则、错误响应、超限输入 |
| F10 | SQL 怎样保存并约束数据？ | F03、基础终端 | 表、约束、参数化查询；拒绝坏数据 |
| F11 | 页面、接口与数据库如何接通？ | F07–10 | 真正的创建/列表/修改/重启后数据仍在 |
