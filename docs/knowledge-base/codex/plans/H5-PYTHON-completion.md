...
- [ ] C 测试/迭代/综合处理三篇、故障实验与发布。
- [ ] 综合验收、台账/STATE、最终提交与同SHA线上检查。

## 恢复与决策

先核对实际Git、STATE、BACKLOG与本计划进度，保护新修改；不要重跑初始化。临时目录只清理自身资源，指针失效按README重建。已有H5-001完整验收保留；当前下一项H5-002A。回合结束保存恢复点，不承诺离线运行。完成Python后停下等待用户确认后续领域。
