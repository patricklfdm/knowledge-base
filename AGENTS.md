...
当前用户追加（2026-09-12）：已明确“恢复浏览器测试”，解除上述暂停。按集中三目标与有限重试执行；持续自动普通push授权保持。若工具环境故障如实记录，不把授权恢复当UI验收通过。

最新用户覆盖（2026-09-13）：浏览器测试再次后移，待规划内容建设完成后统一进行。内容任务按实际先修继续，不受H4 UI阻塞；自动普通push不变。当前不连接/安装/重试浏览器，具体内容范围及依赖见ROADMAP/BACKLOG。

2026-09-13执行检查点：H7-002适用本地验收已完成，H5–H7既定内容依赖满足；依用户上述条件进入集中浏览器阶段，按UI_ACCEPTANCE最多三目标/有限重试。UI与v1.0仍需真实证据，自动普通push持续。

2026-09-13收尾检查点：H0–H7既定任务均已验收，集中三目标与同SHA发布证据见reports/H4-ui-acceptance.md。后续从STATE/MAINTENANCE及真实反馈进入维护，不自动扩张课程；三目标外的未测范围保持NOT_RUN。自动普通push授权持续。
