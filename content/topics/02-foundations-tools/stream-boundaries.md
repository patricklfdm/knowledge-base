...
本篇只使用受控内存流，未覆盖TCP断连恢复、压缩炸弹或生产文件导入。下一篇继续[文件替换](file-publication-boundaries.md)，再进入日志重放和缓存状态；已有[Java资源关闭](../01-languages/java-files-and-resources.md)提供了另一种语言中的所有权对照。
