...
复现本仓库完整测试时，请对齐`.nvmrc`和`.node-version`固定的24.21.0；上面“24系列”只说明练习所选主版本，不能据此推断任意24.x都已通过全库验收。2026-09-14复核时，[官方24.21.0发布记录](https://nodejs.org/en/blog/release/v24.21.0)将它标为LTS；[发布生命周期](https://nodejs.org/en/about/previous-releases)描述的是版本系列的维护阶段，不是每个补丁永久受维护的保证。
