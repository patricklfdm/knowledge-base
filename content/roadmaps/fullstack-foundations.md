...
先完成这三个连续问题，再继续下面的应用目标。文章的核验状态表示教材已复核，不表示读者已经掌握。
...
完整例子位于仓库 `examples/foundations/`，可从根目录执行 `npm test --prefix examples/foundations`。后续应用尚未实现，不把当前终端练习当成已经能保存数据的产品。
