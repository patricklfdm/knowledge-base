...
F10这一节的文件持久化证据仅覆盖单机正常提交、关闭及新进程读取；事务、双连接和备份恢复现已在下述S01–S08扩展验证。断电、磁盘损坏、HTTP/UI整合仍不在本包已测范围。DatabaseSync为同步接口，不适合直接据此推导高并发服务设计。浏览器NOT_RUN：用户批准集中验收。验收见reports/H3-sql.md。
