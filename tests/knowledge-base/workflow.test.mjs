...
test("教学示例单独登记，Quartz 类型范围不包含示例", () => {
  ...
  assert.equal(
    ...
    "npm test --prefix examples/foundations && npm test --prefix examples/typed-trips && npm test --prefix examples/web-forms && npm test --prefix examples/http-trips && npm test --prefix examples/trip-api && npm test --prefix examples/sql-trips",
  )
